...
void save_features(
  ...
  assert(points.size() == features.size() &&
         features.size() == features_pixel.size() &&
         features_pixel.size() == features_velocity.size());
  std::ofstream save_points;
  save_points.open(filename.c_str());
  save_points.setf(std::ios::fixed, std::ios::floatfield);
  save_points.precision(10);
  ...
  for (int i = 0; i < points.size(); ++i) {
    ...
    Eigen::Vector2d pixel = features_pixel[i];
    Eigen::Vector2d velocity = features_velocity[i];
    save_points << p(0) << " " << p(1) << " " << p(2) << " " << p(3) << " "
                << f(0) << " " << f(1) << " " << pixel(0) << " " << pixel(1)
                << " " << velocity(0) << " " << velocity(1) << std::endl;
  }
}
void save_lines(
    std::string filename,
...
void save_Pose(std::string filename, std::vector<MotionData> pose) {
  ...
  save_points.precision(10);
  if (!save_points) {
    printf("file open failed, file path:%s\n", filename.c_str());
    return;
  }
